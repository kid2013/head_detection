...
image get_segmentation_image(char *path, int w, int h, int classes)
{
    ...
    find_replace(labelpath, "Images", "mask", labelpath);
    find_replace(labelpath, ".jpg", ".txt", labelpath);
    find_replace(labelpath, ".JPG", ".txt", labelpath);
    find_replace(labelpath, ".jpg", ".txt", labelpath);
...
{
  /* printf("get_segmentation_image2\n"); */
    char labelpath[4096];
    find_replace(path, "images", "mask", labelpath);
    find_replace(labelpath, "Images", "mask", labelpath);
    find_replace(labelpath, ".jpg", ".txt", labelpath);
...
data load_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
  /* printf("load_data_detection\n"); */
    ...
    d.X.vals = calloc(d.X.rows, sizeof(float*));
    d.X.cols = h*w*3;
    d.file_path = calloc(n, sizeof(char*));
    int r_idx;
    for(r_idx = 0; r_idx < n; ++r_idx){
      d.file_path[r_idx] = random_paths[r_idx];
}
...
char ** concat_path(char **p1, char ** p2, int n1, int n2){
  int n = n1 + n2;
...
data concat_data(data d1, data d2)
{
    ...
    d.w = d1.w;
    d.h = d1.h;
    d.file_path = concat_path(d1.file_path, d2.file_path, n1, n2);
    return d;
}
...
data get_data_part(data d, int part, int total)
{
    data p = {0};
    p.shallow = 1;
    p.file_path = d.file_path + d.X.rows*part/total;
